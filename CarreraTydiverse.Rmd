---
title: "Boletín Tema 4. Preguntas Cortas.MARCELINO MARTINEZ"
subtitle: Tratamiento de Datos. Grado en Ciencia de Datos- UV
author: "Lidia Moreno Marín"
date:  "`r Sys.Date()`"  #11/03/2022
params:
  lang: ES
...
Todas las preguntas están referidas al fichero **YYYY-MM-DD.csv**, que contiene información de los registros (logs) de las descargas realizadas del repositorio CRAN en esa fecha. La información contenida en el fichero es la siguiente:
...
```{r,eval=FALSE}
mydf <- read_csv("./data/2022-03-08.csv")

set.seed(seed=11032022)
N<-200000
I<-sample(1:nrow(mydf),size=N,replace=FALSE)
mydf<-mydf[I,]
# Guardamos en formato binario, la primera vez que cargamos
save(file = './data/2022-03-08.Rdata',list=c('mydf'))
```
...
Ten en cuenta que el conjunto de datos tiene más de **3 millones de registros**
...
```{r}
cran3 <- cran2 %>% select(ip_id, package, size)
```
...
```{r}
cran %>% summarise(avg_size = mean(size))
```
# Agrupamientos
